...
module.exports = (app, data) => {
    return new Promise((resolve, reject) => {
        ...
        function execute(key, method) {
            ...
            if (_.isArray(item)) {
                item.forEach(x => promises.push(method(x).catch(error => handleError(error, x, key))));
            }
            else {
                promises.push(method(item).catch(error => handleError(error, item, key)));
            }
...
function handleError(error, item, key) {
    const message = error.message;

    if (item.qName) {
        error.message = `${key}: ${item.qName}, ${error.message}`;
    }
